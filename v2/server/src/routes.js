...
router.get('/business-brain', authenticate, (req, res) => {
  ...
  const brain = {
    ...
    services: (() => { try { let _v = JSON.parse(row.services || '[]'); return typeof _v === 'string' ? JSON.parse(_v) : _v; } catch(e) { return []; } })(),
    packages: (() => { try { let _v = JSON.parse(row.packages || '[]'); return typeof _v === 'string' ? JSON.parse(_v) : _v; } catch(e) { return []; } })(),
    ...
    faqs: (() => { try { let _v = JSON.parse(row.faqs || '[]'); return typeof _v === 'string' ? JSON.parse(_v) : _v; } catch(e) { return []; } })(),
    scenarios: (() => { try { let _v = JSON.parse(row.scenarios || '[]'); return typeof _v === 'string' ? JSON.parse(_v) : _v; } catch(e) { return []; } })(),
    ai_name: row.ai_name,
    ai_tone: row.ai_tone,
...
router.post('/business-brain', authenticate, (req, res) => {
  ...
  if (existing) {
    ...
    `).run(
      ...
      (() => { let _h = typeof b.hours === 'string' ? JSON.parse(b.hours) : (b.hours || {}); const _hn = {}; Object.keys(_h).forEach(k => { _hn[k.toLowerCase()] = _h[k]; }); return JSON.stringify(_hn); })(), b.ramadan_hours, b.ramadan_enabled ? 1 : 0,
