    ...
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Recent errors and issues — shown in plain language</p>
        </div>
        <button
          onClick={loadLogs}
          disabled={loading}
      ...
      {!loading && !error && logs.length === 0 && (
        <div className="bg-white border border-gray-100 rounded-2xl p-12 text-center shadow-sm">
          ...
          <p className="text-sm text-gray-500">No errors recorded. Your system is running smoothly.</p>
        </div>
      )}
          ...
          {logs.map((log, idx) => {
            ...
            return (
              ...
              >
                ...
                {expanded && (
                  <div className="px-4 pb-4 border-t border-gray-50 pt-3 ml-5 space-y-2">
                    {log.help && (
                    ...
                    {log.technicalMessage && (
                      <div className="text-xs text-gray-400">
                        <span className="font-medium text-gray-500">Technical detail:</span>{' '}
                        <code className="bg-gray-100 px-1.5 py-0.5 rounded">{log.technicalMessage}</code>
                      </div>
                    )}
                    {log.requestPath && (
                      <div className="text-xs text-gray-400">
                        <span className="font-medium text-gray-500">Endpoint:</span> {log.requestPath}
                      </div>
                    )}
                    {log.errorId && (
                      <div className="text-xs text-gray-400">
                        <span className="font-medium text-gray-500">Error ID:</span>{' '}
                        <code className="bg-gray-100 px-1.5 py-0.5 rounded">{log.errorId}</code>
                      </div>
                    )}
                  </div>
                )}
